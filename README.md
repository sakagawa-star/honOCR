...
- **字形**: MinerU が日本語字のかわりに出力する中国語字 21 種を、句読点スタイルによらず常に置換する（簡体字・繁体字 16 種と旧字体 5 種）
- **警告（旧字体）**: 旧字体を置換した箇所を、置換**前**の文脈つきで標準エラーへ報告する。旧字体は固有名詞では正当な表記になりうるため（例: 社名の「廣済堂」）、報告を確認し、必要なら `apply_fixes.py` で元に戻す
- **警告（JIS 外）**: 置換後も残る JIS X 0208 外の漢字を標準エラーへ報告する（字形の対応関係が成立しない個別の誤認識。`apply_fixes.py` で対処する）
