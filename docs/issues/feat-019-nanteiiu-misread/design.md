# feat-019 機能設計書: chap03 の誤読「なんていいう → なんていう」2件の修正

対象案件: `docs/issues/feat-019-nanteiiu-misread/`
...
| FR-002 「なんていいう」の誤読の訂正 | §4.1（`old` / `new` の設計）・§7 手順1 |
...
chap03 は `run-01` のみが存在する（2026-09-02 実測: `{BASE2}/ocr/mineru-full/chap03/` の内容は
...
| `tests/test_*.py`（テストコード） | コード変更がないため、テストの追加・変更も生じない。ただし `tests/results/feat-019_test_result.txt` は検証記録として**新規作成する**（CLAUDE.md「テスト」のルール。§7 手順4） |
...
```json
{
  "fixes": [
    ...
    {
      "id": "chap03-004",
      "reason": "3.5.4 節の本文（原本 page-19_1L.tif）で、原本の「なんていうことはありません」が「なんていいうことはありません」と誤読されていた（「い」が1文字余分。原本 TIF 目視確認済み）。同じ行にある chap03-005 と区別するため、係助詞「は」を含む「ことはありません」まで含めて一意にしている（feat-019）",
      "old": "なんていいうことはありません",
      "new": "なんていうことはありません"
    },
    {
      "id": "chap03-005",
      "reason": "3.5.4 節の本文（原本 page-19_1L.tif）で、原本の「なんていうこともありません」が「なんていいうこともありません」と誤読されていた（「い」が1文字余分。原本 TIF 目視確認済み）。同じ行にある chap03-004 と区別するため、係助詞「も」を含む「こともありません」まで含めて一意にしている（feat-019）",
      "old": "なんていいうこともありません",
      "new": "なんていうこともありません"
    }
  ]
...
**注意**: 上の `chap03-001`〜`chap03-003` は 2026-09-02 時点の既存ファイルの内容を
...
- `old` を「なんていいう」だけにすると、同一行に2件あるため一意にならず、また `new`
  ...
  そのため**両方に文脈を付けて一意化する**
- 2件は「こと**は**ありません」/「こと**も**ありません」の係助詞で区別できる。
...
2026-09-02 に `{FINAL}/chap03_gray300.md`（= `{NORM}` の md とバイト同一）で実測した。
**新規2件を逐次適用したうえで**、既存3件を含む5件すべての最終不変条件を検査している。
...
| fix | 文字列 | 適用後（実測） |
|---|---|---|
| `chap03-001` | `old` = `等しいなんという勘違い` / `new` = `等しいなんていう勘違い` | 0 / **1** |
| `chap03-002` | `old` = `3.5n に近づいていく、なんということはない` / `new` = `3.5n に近づいていく、なんていうことはない` | 0 / **1** |
| `chap03-003` | `old` = `n/2 に近づいていく、なんということはない` / `new` = `n/2 に近づいていく、なんていうことはない` | 0 / **1** |
...
（`/tmp/claude-1000/-home-sakagawa-git-honOCR/{session-id}/scratchpad/feat019/`）を指す。
成果物ディレクトリ（`{BASE2}` 配下）とリポジトリの**外**であり、実装の冒頭で作成する。

```bash
SCRATCH=/tmp/claude-1000/-home-sakagawa-git-honOCR/{session-id}/scratchpad/feat019
mkdir -p "$SCRATCH"
```
...
作られてしまう）。**保存期間はセッション中のみ**であり、
本案件の検証（§7 手順2・手順3）が完了すれば破棄してよい。恒久的な記録は
`tests/results/feat-019_test_result.txt` と案件ドキュメントに残す。
...
また、更新前の md と `{FINAL}` 全体を `{SCRATCH}`（シェル変数 `$SCRATCH`）に退避する（§7 手順2 の `diff` と、
手順3 の失敗時の復元に使う）。

```bash
cp /home/sakagawa/work/確率統計/ocr/mineru-full/chap03/run-01-normalized/chap03_gray300.md \
   "$SCRATCH/chap03_gray300.md.before"
cp -a /home/sakagawa/work/確率統計/ocr/final/chap03 "$SCRATCH/final_chap03.before"

# 退避の成功確認（いずれも一致すること）
sha256sum /home/sakagawa/work/確率統計/ocr/mineru-full/chap03/run-01-normalized/chap03_gray300.md \
          "$SCRATCH/chap03_gray300.md.before"
diff -r /home/sakagawa/work/確率統計/ocr/final/chap03 "$SCRATCH/final_chap03.before" && echo BACKUP_OK
```

- `md.before` の SHA-256 が原本と一致すること
- `diff -r` が無出力で `BACKUP_OK` が出ること（`{FINAL}` 全体＝md・content_list.json・
  images/ 66ファイルが退避できたこと）
...
**`{FINAL}` が新旧混在の部分更新状態で残りうる**。この状態を放置してはならない。

手順3 が終了コード 0 以外で終わった場合の対応:

1. **その場で再実行やリトライをしない**
2. 手順0 で退避した `"$SCRATCH/final_chap03.before"` から `{FINAL}` を復元する

   ```bash
   rm -rf /home/sakagawa/work/確率統計/ocr/final/chap03
   cp -a "$SCRATCH/final_chap03.before" /home/sakagawa/work/確率統計/ocr/final/chap03
   diff -r "$SCRATCH/final_chap03.before" /home/sakagawa/work/確率統計/ocr/final/chap03 && echo RESTORED
   ```

3. **`{NORM}` 側の md も退避から復元する**（下記の理由により必須）

   ```bash
   cp "$SCRATCH/chap03_gray300.md.before" \
      /home/sakagawa/work/確率統計/ocr/mineru-full/chap03/run-01-normalized/chap03_gray300.md
   ```

4. 復元後、`{NORM}` と `{FINAL}` が**再びバイト同一**であることを確認する
   （手順0 の事前確認が通る状態に戻ったことの検証）

   ```bash
   cmp /home/sakagawa/work/確率統計/ocr/mineru-full/chap03/run-01-normalized/chap03_gray300.md \
       /home/sakagawa/work/確率統計/ocr/final/chap03/chap03_gray300.md && echo NORM_FINAL_IDENTICAL
   ```

   あわせて、両者の「なんていいう」が 2 件・文字数が 54009 に戻っていること（＝手順0 の
   期待値と一致すること）を `str.count()` で確認する

5. `RESTORED` と `NORM_FINAL_IDENTICAL` を確認したうえで、**何が起きたか・`{NORM}` と `{FINAL}` の
   両方を修正前の状態に復元したことを報告して中断する**

**`{NORM}` も必ず復元すること。** `apply_fixes.py` は冪等なので `{NORM}` を修正済みのまま
残しても再適用自体は安全に見えるが、その状態では `{NORM}`（修正済み）と `{FINAL}`（修正前）が
バイト同一でなくなり、**手順0 の事前確認「`md bytes identical` が `True` であること」で必ず
中断する**ため、次回の実行を再開できない。両方を修正前に戻すことで、手順0 から素直に
やり直せる状態になる。
...
```bash
diff "$SCRATCH/chap03_gray300.md.before" \
     /home/sakagawa/work/確率統計/ocr/final/chap03/chap03_gray300.md
```
...
`docs/CHANGELOG.md`・`docs/PROJECT_KNOWLEDGE.md`・**`CLAUDE.md`**・ルートの `README.md`）に
変更があってはならない。

| パス | 状態 | 理由 |
|---|---|---|
| `docs/issues/feat-019-nanteiiu-misread/` | `??`（未追跡） | 案件ドキュメント。起票時（ステップ1〜3）に作成済み |
| `docs/BACKLOG.md` | `M` | 起票時に feat-019 の行（ステータス `In Progress`）を追加済み |
| `tests/results/feat-019_test_result.txt` | `??`（未追跡） | 手順4 で新規作成する |
...
```bash
uv run pytest -v > tests/results/feat-019_test_result.txt 2>&1
```

- コード変更がないため、feat-017 完了時点（**216 passed**）と同じくすべて成功することを確認する
- 上記コマンドは出力を `tests/results/feat-019_test_result.txt` に**保存しながら**実行する
...
これは feat-013 §6.1・feat-016 §8・feat-017 design.md §8 で許容済みの既存ポリシーであり、
...
| `build_final.py` の3検証のいずれかが不合格、またはコピー途中で失敗 | 終了コード 1。**`{FINAL}` が部分更新状態で残りうる**（ディレクトリ全体としては原子的でないため） | §6 手順3 の「失敗時の復元」に従い、退避から **`{FINAL}` と `{NORM}` の md の両方**を復元する（`{NORM}` を戻さないと手順0 のバイト同一確認で必ず中断し再開できない）。復元後に両者がバイト同一であることを確認し、復元したことを明記して中断・報告する |
| 手順0 の退避（md・`{FINAL}` のコピー）に失敗 | — | 手順1 に進まず中断して報告する（復元手段がない状態で書き換えないため） |
...
4. 検証まで実施（§6 の手順A・手順0〜3 と §7 の手順1〜4、
   `tests/results/feat-019_test_result.txt` への保存）。**手順0 の退避（md と `{FINAL}` 全体）を必ず先に行い、
   `build_final.py` が失敗した場合は §6 手順3 の「失敗時の復元」に従って `{FINAL}` を復元してから報告する**
...
| ファイル | 更新内容 |
|---|---|
| `docs/BACKLOG.md` | feat-019 のステータスを Closed に更新する（行は起票時に追加済み） |
| `docs/CHANGELOG.md` | 完了内容を記録する |
| `docs/PROJECT_KNOWLEDGE.md` | ドメイン知識の「語彙単位の誤り」の項に、**同じ語の表記ゆれ・挿入脱落のバリアントも正規表現で網羅的に洗い出すこと**を追記する（feat-017 で「なんという」のみを検索して本件を見落とした教訓。案件 README.md §5.1）。**追記には案件 ID「（feat-019）」を付す**（update-003 の非対称ルール）。ディレクトリ構成の変更はない |
| **`CLAUDE.md`** | **更新しない。** update-003 で確定した非対称ルールにより、CLAUDE.md 本体の変更は update 案件でのみ行う。本案件の知見は `docs/PROJECT_KNOWLEDGE.md` に追記する |
...
### ADR-3: MinerU と `normalize_punct.py` を再実行しない
...
### ADR-4: 教訓の追記先を `docs/PROJECT_KNOWLEDGE.md` とし、`CLAUDE.md` を変更しない

- **決定**: バリアント網羅調査の教訓（§11）は `docs/PROJECT_KNOWLEDGE.md` のドメイン知識に
  案件 ID 付きで追記し、`CLAUDE.md` は1文字も変更しない
- **理由**: update-003 で確定した非対称ルール（CLAUDE.md 本体の変更は update 案件でのみ、
  知識ファイルの内容は各案件の完了処理で更新）に従う。本件の教訓は OCR 誤りの類型と
  調査手法に関する**知識**であり、統治文書に置くべき内容ではない
- **代替案**: `CLAUDE.md` のドメイン知識に追記する → update-003 以前の構成では
  そこがドメイン知識の置き場所だったが、外出し済みのため参照先が存在しない。不採用

### ADR-5: feat-018（中止案件）のドキュメントを流用しない

- **決定**: 調査・原本確認・一意性検証・基準値の測定をすべて本案件として 2026-09-02 に
  独立に実施し、feat-018 のドキュメントからの転記を行わない
- **理由**: feat-018 はユーザー指示により中止された案件であり、そのドキュメントを流用しては
  ならないことが案件 README.md（feat-018）に明記されている。とくに feat-018 の
  `requirements.md` §1 と `design.md` §11 の **`CLAUDE.md` 更新の記載はインシデント2 の対象**
  であり、本案件に持ち込んではならない（本案件では ADR-4 のとおり `CLAUDE.md` を変更しない）
- **代替案**: 検証済みの数値を流用して調査を省略する → 中止案件の成果物に依存することになり、
  また本案件独自の検証記録が残らない。不採用
