...
- 上記の不一致のうち**語彙単位の誤読4件は 2026-09-05 に修正済み**（feat-022）。chap01 `? 1.8`（「いい」→「い」）/ chap02 `? 2.10`（「これ」→「こ」）/ chap03 `? 3.5`（「賭けろ」→「賭ける」）/ chap08 `? 8.6`（「だろ」→「だる」）。修正は md のみで、**`content_list.json` には誤読が残る**（既存ポリシー。content_list を基準に走査する後続案件は、md 側が修正済みであることを前提に読むこと）。chap08 `? 8.6` の `##` 欠落は feat-020 で解消済み（feat-022）
- **Q&A コラム見出し74件は、2026-09-05 に全件が md 上で `## ? N.M …` の見出しになった**（feat-020。適用前は 55件）。内訳は α3（`?` 欠落）・β12（`##` 欠落）・γ1（両方欠落）・構造復元3（数式ブロック・HTML `div`・コードブロックからの取り出し）。**`content_list.json` 側は無改変**のため、md と json の乖離は Q&A 見出しで計19件（feat-014 の3件・feat-022 の4件と併せて重複を含む）に及ぶ。**content_list を基準に Q&A 見出しを走査する後続案件は、md 側が修正済みであることを前提に読むこと**（feat-020）
- **Q&A 見出しの「意味に影響する数式記号の欠落」2件は 2026-09-06 に修正済み**（feat-023）。chap06 `? 6.3`（推定量のハット `$\hat{\theta}$` が素の `θ` になっていた）と chap08 `? 8.3`（ベクトルの太字 `$\boldsymbol{q}_{i}\cdot\boldsymbol{x}$` が素の記号になっていた）。**対象2行は原本と完全一致させた**（平文だった `$\theta$`・`$i$` も数式にした）。これにより、feat-021 が整理した**「インライン数式のマークアップ欠落」（`D4`）の残件は 18 → 16 件**になった（feat-023）
...
- **MinerU は Q&A コラム見出しを、見出しではない別種のブロックに取り込むことがある**（feat-020）。実例（確率統計）: `? 2.5` は**数式ブロック `$$…$$` の中に `\text{}` で**、`? 4.6` は **HTML `<div class="mineru-algorithm">`（`content_list` では `type=code` / `sub_type=algorithm`）の中に**、`? 7.3` は**コードブロック ` ```txt ` の中に質問と答え全体が**取り込まれていた。**行頭に `? N.M` が見えても `##` を付けるだけでは見出しにならない**（ブロックごと解体する必要がある）。`apply_fixes.py` は複数行の `old`/`new` を扱えるため、開始・終了の区切り（`$$`・`<div>`/`</div>`・フェンス）を含めて置換すれば解体できる。**解体後の痕跡を数える受け入れ基準を作るときは、同じ区切り記号が他所に正当に存在しないかを必ず数えること**（feat-020 では chap07 の ` ```txt ` を「0件になる」と誤って基準化し、実装フェーズで発覚した。実際は Q&A と無関係な正当なコードブロックが4つあり 5→4 に減るのが正しい）
- **同じ章に複数の run（`run-01` / `run-02` 等）がある場合、`final/` のコピー元がどれかを必ず確認する**（feat-020）。確率統計 chap07 は `run-01` と `run-02` があり、**`final/chap07` は `run-02-normalized` とバイト同一**である（`run-01-normalized` の md とは不一致）。コピー元でない run に `apply_fixes.py` を適用しても final に反映されず、次回の final 再構築で失われる。`{NORM}` の md と `final` の md がバイト同一であることを事前確認に入れると取り違えを検出できる
...
| 2026-09-05 | 120 | 検討不要 | feat-020 のデータの状態1行・ドメイン知識2行を追記。500 行未到達 |
| 2026-09-06 | 122 | 検討不要 | feat-023 のデータの状態1行を追記。500 行未到達 |
